// app/api/payment-processors/opay/buyer-credentials/status/route.ts
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: credentialCheck.valid
        ? 'OPay buyer credentials are valid'
        : 'OPay buyer credentials are invalid or expired',
    });
  } catch (error) {
    console.error('[OPay] Error checking buyer credential status:', error);
