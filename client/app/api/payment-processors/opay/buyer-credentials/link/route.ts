// app/api/payment-processors/opay/buyer-credentials/link/route.ts
//
// Links a buy order PDA → OPay buyer credential.
// Upserts into the existing buy_order_credentials table (no new table needed).
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!credential) {
      return NextResponse.json(
        { error: 'Invalid or inactive OPay buyer credential' },
        { status: 400 }
      );
    }
    ...
    if (error) {
      console.error('[OPay] DB error linking buyer credential:', error);
      return NextResponse.json({ error: 'Failed to link credential' }, { status: 500 });
    }

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('[OPay] Error linking buyer credential:', error);
