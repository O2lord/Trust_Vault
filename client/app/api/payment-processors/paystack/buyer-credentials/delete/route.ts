// app/api/payment-processors/paystack/buyer-credentials/delete/route.ts
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (linked && linked.length > 0) {
      return NextResponse.json({ error: 'Cannot delete: credential is linked to active buy orders' }, { status: 400 });
    }
