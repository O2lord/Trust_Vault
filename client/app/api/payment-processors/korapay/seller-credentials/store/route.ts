// app/api/payment-processors/korapay/seller-credentials/store/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    const credentialCheck = await KorapayService.validateCredentials({ secretKey });
    if (!credentialCheck.valid) {
      return NextResponse.json({ error: credentialCheck.error ?? 'Invalid Korapay credentials' }, { status: 400 });
    }
    ...
    if (error) {
      console.error('[Korapay] DB error storing seller credentials:', error);
    ...
    return NextResponse.json({
      ...
      message: 'Korapay seller credentials stored successfully',
    });
  } catch (error) {
    console.error('[Korapay] Error storing seller credentials:', error);
