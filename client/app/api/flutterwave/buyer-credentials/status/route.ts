// api/flutterwave/buyer-credentials/status/route.ts
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: credentialCheck.valid
        ? 'Buyer credentials are valid'
        : 'Buyer credentials are invalid or expired',
    });
  } catch (error) {
    console.error('Error checking buyer credential status:', error);
    return NextResponse.json(
