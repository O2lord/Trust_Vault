...
function getOrderType(receipt: Receipt, walletAddress: string): 'buy' | 'sell' {
  const isTaker = receipt.taker_address === walletAddress;
  const isIpOrder = receipt.payout_reference.startsWith('IP-');
  
  if (isTaker) {
    // From user's perspective: IP = they sold tokens, IS = they bought tokens
    return isIpOrder ? 'sell' : 'buy';
  } else {
    // LP perspective: IP = buy order, IS = sell order
    return isIpOrder ? 'buy' : 'sell';
  }
}
