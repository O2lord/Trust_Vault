...
interface UseBuyerFlutterwaveCredentialsReturn {
  credentials: BuyerCredential[];
  loading: boolean;
  fetchCredentials: () => Promise<void>;
  addCredential: (secretKey: string, label?: string) => Promise<boolean>;
  deleteCredential: (credentialId: string) => Promise<boolean>;
  linkToBuyOrder: (trustExpressPda: string, credentialId: string) => Promise<boolean>;
}

const useBuyerFlutterwaveCredentials = (): UseBuyerFlutterwaveCredentialsReturn => {
  const { publicKey, signMessage } = useWallet();
  const [credentials, setCredentials] = useState<BuyerCredential[]>([]);
  ...
  const fetchCredentials = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const [flwRes, opayRes, paystackRes, korapayRes] = await Promise.allSettled([
        fetch(`/api/flutterwave/buyer-credentials/list?${params}`),
        fetch(`/api/payment-processors/opay/buyer-credentials/list?${params}`),
        fetch(`/api/payment-processors/paystack/buyer-credentials/list?${params}`),
        fetch(`/api/payment-processors/korapay/buyer-credentials/list?${params}`),
      ]);

      const merged: BuyerCredential[] = [];
  ...
  const linkToBuyOrder = useCallback(async (
    ...
    try {
      const message = generateAuthMessage('link_buy_order_credential');
      const signature = await signAuthMessage(message);
      ...
      const endpoint =
        credential?.processor === 'korapay'
          ? '/api/payment-processors/korapay/buyer-credentials/link'
          : credential?.processor === 'paystack'
            ? '/api/payment-processors/paystack/buyer-credentials/link'
            : credential?.processor === 'opay'
              ? '/api/payment-processors/opay/buyer-credentials/link'
              : '/api/flutterwave/buyer-credentials/link';
...
export default useBuyerFlutterwaveCredentials;