...
import { AlertCircle, CheckCircle2, Eye, EyeOff, Loader2, Plus, Trash2, RefreshCw, Shield, Store } from 'lucide-react';
...
const SellerFlutterwaveCredentialManager = () => {
  ...
  const fetchCredentials = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/flutterwave/seller-credentials/list?${params}`);
  ...
  const saveCredential = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        toast.success(data.hasSubaccount 
          ? 'Credentials saved with subaccount!' 
  ...
  const checkStatus = async (credentialId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/flutterwave/seller-credentials/status?${params}`);
      const data = await response.json();
  ...
  if (!publicKey) {
    return (
      <Card className="bg-[#F5F0E8] border-[#C8C2B4]">
        <CardHeader>
          <CardTitle className="text-[#0F0D0A] flex items-center gap-2">
            <Store className="w-5 h-5 text-blue-500" />
            Seller Flutterwave Credentials
          </CardTitle>
          <CardDescription className="text-[#6B6558]">
            Connect your wallet to manage your seller credentials
          </CardDescription>
        </CardHeader>
      </Card>
    );
  ...
  return (
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-[#6B6558]">
                Manage your Flutterwave credentials for receiving payments
              </CardDescription>
            </div>
        ...
        <CardContent>
          <div className="bg-blue-900/50 border border-blue-800 rounded-lg p-4 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <p className="text-sm text-blue-200 font-medium">For Sell Orders Only</p>
                <p className="text-xs text-blue-300">
                  These credentials are used to generate payment links and monitor incoming payments for your sell orders.
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-[#6B6558]">
              ...
              <p className="text-sm">Add your Flutterwave secret key to enable payment links</p>
            </div>
          ) : (
                ...
                <div key={credential.id} className="bg-[#EDE8DF] rounded-lg p-4 flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {credential.flutterwave_subaccount_id && (
      ...
      {/* Add Credential Dialog - Same as before but with seller-specific messaging */}
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="bg-[#F5F0E8] border-[#C8C2B4] max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#0F0D0A]">Add Seller Flutterwave Credential</DialogTitle>
            <DialogDescription className="text-[#6B6558]">
              Enter your Flutterwave secret key to enable payment link generation for sell orders
            </DialogDescription>
          </DialogHeader>
                  ...
                  {testResult.valid ? (
                    <>
                      ...
                      <div>
                        ...
                        {testResult.hasSubaccount && (
                          <p className="text-xs text-green-300">✓ Subaccount will be created</p>
                        )}
                      </div>
                    </>
                  ) : (
          ...
          <DialogFooter>
            ...
            <Button onClick={saveCredential} disabled={saving || !secretKey.trim()} className="bg-blue-600 hover:bg-blue-700">
              {saving ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                  Saving...
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={() => setDeleteConfirmId(null)}>
        <DialogContent className="bg-[#F5F0E8] border-[#C8C2B4]">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#6B6558]">
              This action cannot be undone. Make sure this credential is not being used by any active sell orders.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
export default SellerFlutterwaveCredentialManager;