...
const FlutterwaveCredentialManager = () => {
  ...
  const checkStatus = async (credentialId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setCredentials(prev => prev.map(c => 
          c.id === credentialId 
            ? { ...c, checking: false, balance: data.balance, currency: data.currency, is_active: data.valid } 
            : c
        ));
  ...
  if (!publicKey) {
    return (
      <Card className="bg-[#F5F0E8] border-[#C8C2B4]">
        <CardHeader>
          <CardTitle className="text-[#0F0D0A]">Flutterwave Credentials</CardTitle>
          <CardDescription className="text-[#6B6558]">
            Connect your wallet to manage Flutterwave credentials
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-[#6B6558]">
                Manage your Flutterwave API credentials securely
              </CardDescription>
            </div>
        ...
        <CardContent>
          <div className="bg-blue-900/50 border border-blue-800 rounded-lg p-4 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <p className="text-sm text-blue-200 font-medium">Secure Storage</p>
                <p className="text-xs text-blue-300">
                  Your credentials are encrypted using AES-256-GCM and stored securely. They are only decrypted when processing payouts.
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-[#6B6558]">
              ...
              <p className="text-sm">Add your first Flutterwave credential to get started</p>
            </div>
          ) : (
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="bg-[#F5F0E8] border-[#C8C2B4] max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#0F0D0A]">Add Flutterwave Credential</DialogTitle>
            <DialogDescription className="text-[#6B6558]">
              Enter your Flutterwave secret key to enable custom payouts
            </DialogDescription>
          </DialogHeader>
...
export default FlutterwaveCredentialManager;