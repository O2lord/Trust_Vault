// components/TrustExpress/CredentialsManager/BuyOrder/PaystackSellerCredentialManager.tsx
...
const PaystackSellerCredentialManager = () => {
  ...
  const fetchCredentials = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/payment-processors/paystack/seller-credentials/list?walletAddress=${publicKey.toBase58()}`);
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/payment-processors/paystack/seller-credentials/store', {
        method: 'POST',
  ...
  const saveCredential = async () => {
    ...
    setSaving(true);
    try {
      const message = genMsg('store_seller_credentials');
  ...
  const deleteCredential = async (credentialId: string) => {
    ...
    try {
      const message = genMsg('delete_seller_credential');
      const signature = await signMsg(message);
      const params = new URLSearchParams({ credentialId, walletAddress: publicKey.toBase58(), signature, message });
      const res = await fetch(`/api/payment-processors/paystack/seller-credentials/delete?${params}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  const checkStatus = async (credentialId: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/payment-processors/paystack/seller-credentials/status?${params}`);
      const data = await res.json();
...
export default PaystackSellerCredentialManager;