...
import { TrendingUp, TrendingDown, ShoppingCart, DollarSign, Activity, Shield, Users } from "lucide-react";
...
export function GlobalStats({ globalState }: GlobalStatsProps) {
  const stats = [
    ...
    {
      title: "Total Volume",
  value: globalState?.totalVolume 
    ? `$${(Number(globalState.totalVolume.toString()) / 1e9).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    : '$0.00',
    ...
    },
    {
      ...
      value: globalState?.totalDisputes?.toString() || '0',
      icon: Shield,
      description: "Disputed trades",
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            {globalState?.validatorCount || 0} / 5
          </div>
          <p className="text-xs text-muted-foreground">
            {globalState?.requiredVotes || '–'}-of-{globalState?.validatorCount || 0} threshold
          </p>
        </CardContent>
      </Card>
