...
import SellerReleaseCard from "./TakerCard";
...
export interface SellerReleaseRef {
  refresh: () => void;
}

const TakerCardContainerBO = forwardRef<SellerReleaseRef>((_, ref) => {
  ...
  // Filter trustVaults that belong to the current user and have pending reservations
  // Now including disputed reservations (status === 4) along with pending ones
  // and filtering for only BUY_FIRST type trustVaults
  const pendingReservations = useMemo(() => {
    ...
    const userTrustVaults = getTrustVaultAccounts.data.filter(trustVault => 
      ...
      trustVault.account.reservedAmounts.some(reservation => 
        reservation.taker.equals(publicKey) && 
        ...
        if (reservation.status === 0 || reservation.status === 1 || reservation.status === 4) {
          reservations.push({
            ...
            reservation: {
              ...
              disputeId: reservation.disputeId,
            },
          });
        }
      });
  ...
  useEffect(() => {
    ...
    fetchMintInfo();
  }, [pendingReservations, getMintInfo, mintInfoCache]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {pendingReservations.map((item) => {
          ...
          const reservationWithType = {
            ...
            paymentInstructions: item.trustVaultAccount.paymentInstructions,
          };
...
TakerCardContainerBO.displayName = "TakerCardContainerBO";

export default TakerCardContainerBO;