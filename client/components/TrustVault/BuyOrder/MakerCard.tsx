...
const MakerCard: React.FC<Props> = ({
  ...
  const { buyOrderPaymentSent } = useTrustVaultProgram();
  ...
  // Check if this is a buy-order trustVault type
  const isValidTrustVault = useMemo(() => {
    if (!trustVaultAccount) {
      return false;
    }
    return trustVaultAccount.trustVaultType === TRUST_VAULT_TYPE_BUY_ORDER;
  }, [trustVaultAccount]);
  
  // If not a buy-order trustVault, don't render the card
  ...
  // Get payment instructions - prioritize reservation-level over trust vault-level
  const getPaymentInstructions = (): string => {
    const reservationData = trustVaultAccount.reservedAmounts?.[reservationIndex];
    let sellerInstructions = reservationData?.sellerInstructions || '';

    // Fallback to trust vault-level instructions if reservation-level doesn't exist
    if (!sellerInstructions) {
      sellerInstructions = trustVaultAccount.sellerInstructions || '';
    }
  ...
  const handleConfirmPaymentSent = async () => {
    toast.promise(buyOrderPaymentSent.mutateAsync({ 
      trustVault, 
      reservationIndex,
      seller: reservation.taker // Pass the seller (taker) from the reservation
    }), {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-red-700 dark:text-red-400">
                ...
                are locked until the dispute is resolved. Please open a ticket in our <a href="https://discord.gg/ErvrS3BPJv" target="_blank" rel="noopener noreferrer"
  ...
  <Card className={getCardBorderClass()}>
    ...
    <CardContent className="space-y-4">
      ...
      {/* Collapsible Payment Instructions Section */}
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50">
        <button
          type="button"
          onClick={() => setIsPaymentInstructionsOpen(!isPaymentInstructionsOpen)}
          className="w-full flex items-center justify-between p-3 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors rounded-t-lg"
        >
          <div className="flex items-center gap-2">
      ...
      <div className="flex gap-2">
        {reservation.status === 0 ? (
          ...
          <Button 
            className="flex-1"
            disabled={buyOrderPaymentSent.isPending}
            onClick={() => setShowConfirmation(true)}
            variant="default"
          >
            <Send className="w-4 h-4 mr-2" />
            {buyOrderPaymentSent.isPending ? "Processing..." : "Mark Payment Sent"}
          </Button>
    ...
    <ConfirmationDialog
      ...
      cancelText="Cancel"
      isProcessing={buyOrderPaymentSent.isPending}
    />
  </Card>
);
};
